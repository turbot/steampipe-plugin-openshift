...
func tableOpenShiftPod(ctx context.Context) *plugin.Table {
	return &plugin.Table{
		Name:        "openshift_pod",
		Description: "Retrieve information about your pods.",
		...
		Columns: commonColumns([]*plugin.Column{
			...
			},
			{
				...
				Type:        proto.ColumnType_STRING,
				Description: "If specified, indicates the pod's priority. 'system-node-critical' and 'system-cluster-critical' are two special keywords which indicate the highest priorities with the former being the highest priority. Any other name must be defined by creating a PriorityClass object with that name.",
			...
			},
			{
				Name:        "phase",
				Type:        proto.ColumnType_STRING,
				Description: "The phase of a Pod is a simple, high-level summary of where the Pod is in its lifecycle. The conditions array, the reason and message fields, and the individual container status arrays contain more detail about the pod's status. There are five possible phase values: Pending, Running, Succeeded, Failed, Unknown",
...
func listPods(ctx context.Context, d *plugin.QueryData, _ *plugin.HydrateData) (interface{}, error) {
	config, err := getClient(ctx, d)
	if err != nil {
		plugin.Logger(ctx).Error("openshift_pod.listPods", "connection_error", err)
		return nil, err
	}
	client, err := client_v1.NewForConfig(config)
	if err != nil {
		plugin.Logger(ctx).Error("openshift_pod.listPods", "NewForConfig_error", err)
		return nil, err
	}
	...
	for {
		response, err := client.CoreV1().Pods("").List(ctx, input)
		if err != nil {
			plugin.Logger(ctx).Error("openshift_pod.listPods", "api_error", err)
			return nil, err
		}
...
func getPod(ctx context.Context, d *plugin.QueryData, _ *plugin.HydrateData) (interface{}, error) {
	...
	config, err := getClient(ctx, d)
	if err != nil {
		plugin.Logger(ctx).Error("openshift_pod.getPod", "connection_error", err)
		return nil, err
	}
	client, err := client_v1.NewForConfig(config)
	if err != nil {
		plugin.Logger(ctx).Error("openshift_pod.getPod", "NewForConfig_error", err)
		return nil, err
	}

	pod, err := client.CoreV1().Pods(namespace).Get(ctx, name, v1.GetOptions{})
	if err != nil {
		plugin.Logger(ctx).Error("openshift_pod.getPod", "api_error", err)
		return nil, err
	}
